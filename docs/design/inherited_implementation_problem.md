...
#     We want child to inherits the implementation from Parent
...
Solve the inheritence problem by just yoinking the abstract method stub out of the ancestor chain, by just using `remove_method()`.
...
Cons:
  ...
  - Calls to implemented abstract methods are faster, at the expense of unimplemented ones
    ...
    - but that's totally acceptable, because this should never happen in a completed program. The performance of calls to actually implemented methods is what matters, since that what the real code will be doing at a huge volume.
...
Cons:
  - Repetitive
  - Can be forgotten
  - There's more overhead on every method call (to abstracts method with inherited implementation):
...
Cons:
  ...
  - To determine if a method if abstract or not, every `sig` needs have its block evaluated, to see if it calls `abstract`.
